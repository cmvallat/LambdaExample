...
```bash
test-second-lambda$ sam local invoke helloFromLambdaFunction --no-event
```
...
```bash
test-second-lambda$ sam logs -n helloFromLambdaFunction --stack-name sam-app --tail
```
...
```bash
test-second-lambda$ dotnet test test/test-second-lambda.Tests/test-second-lambda.Tests.csproj 
```
...
```bash
sam delete --stack-name test-second-lambda
```
